use super::storage::{Storage, Name, Balance};

pub fn compute_most_profitable(storage: &Storage) -> Option<(Name, Balance)> {
    storage.get_all().into_iter().max_by(|(_, lhs), (_, rhs)|{
...
mod tests {
    ...
    fn test_compute_most_profitable(){
        ...
        storage.deposit(&"Alice".to_owned(), Balance::new(100)).unwrap();
        storage.add_user("Bob".to_owned());
        storage.deposit(&"Bob".to_owned(), Balance::new(200)).unwrap();
